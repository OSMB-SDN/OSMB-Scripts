...
public class NightmareZone extends Script {
    ...
    private Task getArenaTask(boolean walking, WorldPosition worldPosition) {
        ...
        Integer prayerPoints = getWidgetManager().getMinimapOrbs().getPrayerPointsPercentage();
        Boolean quickPrayersActive = getWidgetManager().getMinimapOrbs().isQuickPrayersActivated();

        if (quickPrayersActive == null || prayerPoints == null) {
            log(NightmareZone.class, "Unable to read Prayer orb...");
            return null;
        }
        // Check prayers activated
        if (secondaryPotion == StandardPotion.PRAYER_POTION) {
        ...
        boolean canDrink;
        if (statBoostPotion == BarrelPotion.OVERLOAD) {
            if (hitpointsPercentage == 100 && hitpoints < 51) {
            ...
            canDrink = hitpoints >= 51 && !overloadBuffOverlay.isVisible();
        } else {
        ...
        if (flickRapidHeal && rapidHealFlickTimer.hasFinished() && hitpoints == 1) {
            dynamicTasks.add(Task.FLICK_PRAYER);
        }


        if (secondaryPotion == BarrelPotion.ABSORPTION_POTION && hitpoints > 1) {
...
//            }
        ...
        if (specialAttackWeapon != null && specialDelayTimer.hasFinished()) {
            Integer specialAttackAmount = getWidgetManager().getMinimapOrbs().getSpecialAttackPercentage();
            if (specialAttackAmount == null) {
                log(NightmareZone.class, "Special attack orb not visible...");
                return null;
            }
            if (specialAttackAmount >= specialAttackWeapon.getAmount()) {
    ...
    private void activateSpecialAttack() {
        Integer previousPercentageResult = getWidgetManager().getMinimapOrbs().getSpecialAttackPercentage();
        if (previousPercentageResult == null) {
            log(NightmareZone.class, "Problem reading special attack percentage");
            return;
        }
        if (getWidgetManager().getMinimapOrbs().setSpecialAttack(true)) {
            boolean result = pollFramesUntil(() -> {
                Boolean activated = getWidgetManager().getMinimapOrbs().isSpecialAttackActivated();
                if (activated == null) {
                    log(NightmareZone.class, "Problem reading special attack orb");
                    return false;
                }
                return !activated;
            }, 6000);
    ...
    private void flickPrayer() {
        Boolean prayersActivated = getWidgetManager().getMinimapOrbs().isQuickPrayersActivated();
        if (prayersActivated == null) {
    ...
    private void lowerHealth(int timeout) {
        submitTask(() -> {
            Integer healthResult = getWidgetManager().getMinimapOrbs().getHitpoints();
